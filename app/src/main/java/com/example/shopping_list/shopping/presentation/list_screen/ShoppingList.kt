...
import com.example.shopping_list.shopping.presentation.list_screen.components.Items
import com.example.shopping_list.shopping.presentation.list_screen.components.ShoppingEditor
            ...
            items(sItem, key ={ eachItem->eachItem.id} ) { eachItem ->      //we can use a keyword instead of it,this is it
                if(!eachItem.isEditing) {
                    Items(
                        ...
                        eachItem,
                        onClick = {viewModel.setDialog(true)},
                        onEdit = { viewModel.setEditingItem(eachItem.id) },
                        onDelete = { viewModel.removeItem(eachItem) }
                    )
                }
                else{
                    ShoppingEditor(eachItem) { newName, newQty ->
                       viewModel.updateItem(eachItem.id, newName, newQty, address=address)
                    }
                }
            }
        }
            ...
            confirmButton = {
                Row(modifier = Modifier.fillMaxWidth().padding(1.dp),horizontalArrangement=Arrangement.SpaceBetween) {
                    Button(
                        ...
                        onClick = { viewModel.addItem(address) }
                    ) {
